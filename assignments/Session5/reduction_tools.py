...
class Reduction(ReducerBase):
    ...
    def __init__(self, *arg, **kwd):
        ...
        self._dark_calib = DarkSubtract(
            master_source=self._master_source, dark_scale=allow_dark_scale
        )
    ...
    def action(self):
        ...
        try:
            ...
            print(
                "One or more of the reduced images already exists. Delete "
...
class Combiner(ReducerBase):
    ...
    def action(self):
        ...
        for idx, combo_group in enumerate(groups_to_combine):
            ...
            name_addons = [
                "_".join([str(k), str(v)]) for k, v in six.iteritems(combo_group)
            ]
    ...
    def _action_for_one_group(self, filter_dict=None):
        ...
        combined = ccdproc.combine(
            file_list, mem_limit=DEFAULT_MEMORY_LIMIT, **combine_keyword_args
        )
...
class CalibrationStep:
    ...
    def _master_image(self, selector, closest=None):
        ...
        file_name = self._master_source.files_filtered(master=True, **selector)
        if len(file_name) > 1:
            raise RuntimeError(
                "Well, crap. Should only be one master but "
                "found these matches: "
            ...
            if closest is None:
                ...
                file_name = self._master_source.files_filtered(
                    master=True, **new_select
                )
                master_table = self._master_source.summary
                min_dist = 1e20
                for name in file_name:
                    match = master_table["file"] == name
                    distance = abs(master_table[closest][match] - selector[closest])
        ...
        try:
            ...
            try:
                ...
                self._image_cache[path] = ccdproc.CCDData.read(
                    path, unit=DEFAULT_IMAGE_UNIT
                )
            return self._image_cache[path]
    ...
    def action(self, ccd):
        ...
        if self.scale:
            ...
            if "subbias" not in master.meta:
                raise RuntimeError(
                    "Bias has not been subtracted from dark, so cannot scale dark"
                )
        else:
            try:
                master = self._master_image(select_dict)
